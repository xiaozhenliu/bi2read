...
build_root="${BI2READ_BUILD_ROOT:-$repo_root/target/macos-local}"

case "$build_root" in /*) ;; *) printf 'BI2READ_BUILD_ROOT must be absolute.\n' >&2; exit 2 ;; esac
...
[ ! -e "$app" ] || {
    ...
    printf 'Choose a new BI2READ_BUILD_ROOT or move the existing app.\n' >&2
    exit 1
}
...
if [ ! -d "$runtime_repo/.git" ]; then
    git clone --branch "$runtime_tag" --depth 1 \
        https://github.com/xiaozhenliu/bi2read-funasr-runtime.git \
        "$runtime_repo"
fi
...
identity="${BI2READ_SIGN_IDENTITY:-}"
...
codesign "${sign_args[@]}" "$app/Contents/MacOS/bi2read"
...
"$app/Contents/MacOS/bi2read" --package-self-check
